...
import { Step1TaxFilings } from "./Step1TaxFilings";
import { Step2StateRegistrations } from "./Step2StateRegistrations";
import { Step3Licenses } from "./Step3Licenses";
import { Step4OtherDocuments } from "./Step4OtherDocuments";
import { ComplianceTrackingPreview } from "./ComplianceTrackingPreview";
import { ComplianceData, INITIAL_DATA } from "./types";
...
const WIZARD_STEPS = [
    {
        id: "step-1",
        title: "Tax Filings",
        description: "Federal, State & Local Taxes",
        component: Step1TaxFilings
    },
    {
        id: "step-2",
        title: "Registrations",
        description: "State & Foreign Qualifications",
        component: Step2StateRegistrations
    },
    {
        id: "step-3",
        title: "Licenses",
        description: "Operating Permits & Licenses",
        component: Step3Licenses
    },
    {
        id: "step-4",
        title: "Documents",
        description: "Expiration Tracking",
        component: Step4OtherDocuments
    }
];

interface ComplianceTrackingWizardProps {
    initialData?: ComplianceData;
    onSave?: (data: ComplianceData) => Promise<void>;
    onReset?: (entityId: string) => Promise<any>;
    onComplete?: (data: ComplianceData) => void;
    onCancel?: () => void;
}

export function ComplianceTrackingWizard({ initialData, onSave, onReset, onComplete, onCancel }: ComplianceTrackingWizardProps) {
    ...
    // Auto-Save State (Manual Save Pattern)
    const [saveStatus, setSaveStatus] = useState<SaveStatus>("idle");
    const [lastSaved, setLastSaved] = useState<Date | undefined>(undefined);

    const [data, setData] = useState<ComplianceData>(initialData ? { ...INITIAL_DATA, ...initialData } : INITIAL_DATA);
    ...
    const handleUpdate = useCallback((updates: Partial<ComplianceData>) => {
    ...
    const handleNext = async () => {
        if (currentStep < WIZARD_STEPS.length - 1) {
            ...
            try {
                ...
                toast.success("Compliance Profile Completed", {
                    description: "Your compliance information has been successfully saved."
                });
    ...
    return (
        !isMounted ? <WizardSkeleton /> :
            <ModuleErrorBoundary name="Compliance Tracking Module">
                <TooltipProvider>
                    <div className="w-full max-w-5xl mx-auto p-4 md:p-6 lg:p-8">
                        {/* Header */}
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                            <div>
                                <h1 className="text-3xl font-bold font-display tracking-tight">Compliance Tracking</h1>
                                <p className="text-muted-foreground mt-1">Monitor regulatory requirements and filing deadlines.</p>
                            </div>
                        ...
                        {isPreviewMode ? (
                            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                                <ComplianceTrackingPreview data={data} mode="preview" />
                            </div>
                        ) : (
                        ...
                        <AlertDialog open={showResetConfirm} onOpenChange={setShowResetConfirm}>
                            <AlertDialogContent>
                                <AlertDialogHeader>
                                    <AlertDialogTitle>Delete All Compliance Data?</AlertDialogTitle>
                                    <AlertDialogDescription>
                                        This will permanently delete your compliance profile from our records.
                                    </AlertDialogDescription>
                                </AlertDialogHeader>
                                <AlertDialogFooter>
