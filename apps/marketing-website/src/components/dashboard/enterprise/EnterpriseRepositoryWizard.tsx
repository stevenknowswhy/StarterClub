...
import { Step1InterestedParties } from "./Step1InterestedParties";
import { Step2AccessControl } from "./Step2AccessControl";
import { EnterpriseRepositoryPreview } from "./EnterpriseRepositoryPreview";
import { EnterpriseRepositoryData, INITIAL_ENTERPRISE_DATA } from "./types";

// Consistent SaveStatus type
...
const WIZARD_STEPS = [
    {
        ...
        component: Step1InterestedParties
    },
    {
        id: "step-2",
        title: "Access Control",
        description: "Configure permissions",
        component: Step2AccessControl
    }
];

interface EnterpriseRepositoryWizardProps {
    initialData?: EnterpriseRepositoryData;
    onSave?: (data: EnterpriseRepositoryData) => Promise<void>;
    onReset?: () => Promise<void>;
}

export function EnterpriseRepositoryWizard({ initialData, onSave, onReset }: EnterpriseRepositoryWizardProps) {
    ...
    const [data, setData] = useState<EnterpriseRepositoryData>(initialData || INITIAL_ENTERPRISE_DATA);
    ...
    const handleUpdate = useCallback((updates: Partial<EnterpriseRepositoryData>) => {
    ...
    const handleNext = async () => {
        if (currentStep < WIZARD_STEPS.length - 1) {
            // Optional: validate step before moving?
            ...
            try {
                ...
                toast.success("Configuration Completed", {
                    description: "Your access control settings have been saved."
                });
    ...
    const confirmReset = async () => {
        ...
        try {
            ...
            setData(INITIAL_ENTERPRISE_DATA);
            setCurrentStep(0);
    ...
    if (!isMounted) return <WizardSkeleton />;

    return (
        <ModuleErrorBoundary name="Enterprise Repository Manager">
            <TooltipProvider>
                <div className="w-full max-w-5xl mx-auto p-4 md:p-6 lg:p-8">
                    {/* Header */}
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                        <div>
                            <h1 className="text-3xl font-bold font-display tracking-tight">Enterprise Repository Manager</h1>
                            <p className="text-muted-foreground mt-1">Manage access control for your acquisition readiness documents.</p>
                        </div>
                    ...
                    {isPreviewMode ? (
                        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                            <EnterpriseRepositoryPreview data={data} />
                        </div>
                    ) : (
                    ...
                    <AlertDialog open={showResetConfirm} onOpenChange={setShowResetConfirm}>
                        <AlertDialogContent>
                            <AlertDialogHeader>
                                <AlertDialogTitle>Delete All Configuration Data?</AlertDialogTitle>
                                <AlertDialogDescription>
                                    This will remove all interested parties and their access permissions. This action cannot be undone.
                                </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
